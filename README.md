...
* [Labs](https://github.com/kelseyhightower/kubernetes-the-hard-way/tree/52eb26dad1a3e9e8083a899bc854421eb4842a73#labs) - Original Guide
...
for i in jumpbox server node-0 node-1; do
  associate_public_ip=$([ $i == "jumpbox" ] && echo -n "--associate-public-ip-address" || echo -n "")
...
+---------+-------------+-----------------+----------------+-----------------+------------+
|  jumpbox|  us-east-2c |  t2.micro       |  10.240.0.100  |  18.222.56.80   |  running   |
|  server |  us-east-2c |  t2.micro       |  10.240.0.126  |  None           |  running   |
|  node-0 |  us-east-2c |  t2.micro       |  10.240.0.137  |  None           |  running   |
|  node-1 |  us-east-2c |  t2.micro       |  10.240.0.29   |  None           |  running   |
+---------+-------------+-----------------+----------------+-----------------+------------+
```
...
`machines.txt` file that will be used in the main guide sections.
...
```sh
scp -i ./kubernetes-the-hard-way.id_rsa ./kubernetes-the-hard-way.id_rsa admin@$JUMPBOX_IP:/tmp/
ssh -i ./kubernetes-the-hard-way.id_rsa admin@$JUMPBOX_IP \
  'sudo mv /tmp/kubernetes-the-hard-way.id_rsa /root/.ssh/id_rsa && \
  sudo chown root:root /root/.ssh/id_rsa && sudo chmod 600 /root/.ssh/id_rsa'

for ip in $(cut -d' ' -f1 machines.txt); do
  ssh -i ./kubernetes-the-hard-way.id_rsa \
    -o StrictHostKeyChecking=no \
    -o UserKnownHostsFile=/dev/null \
    -o "ProxyCommand=ssh -i ./kubernetes-the-hard-way.id_rsa \
        -o StrictHostKeyChecking=no \
        -o UserKnownHostsFile=/dev/null \
        -W %h:%p admin@$JUMPBOX_IP" \
    admin@$ip \
    "sudo sed -i \
...
```sh
ssh -t -i ./kubernetes-the-hard-way.id_rsa admin@$JUMPBOX_IP "export LC_ALL=C.UTF-8 LANG=C.UTF-8; sudo -i"
```
